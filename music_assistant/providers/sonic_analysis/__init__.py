...
async def setup(
    ...
    # Never import the heavy torch/transformers stack on a host that cannot run it; a host
    # that fails this check can still use Sonic Analysis entirely through a remote worker.
    if await local_ml_analysis_capable(min_memory_gb=MIN_RAM_GB, min_cpu_cores=MIN_CPU_CORES):
        from .provider import SonicAnalysisProvider  # noqa: PLC0415

        return SonicAnalysisProvider(mass, manifest, config)
    return SonicAnalysisRemoteOnlyProvider(mass, manifest, config, set())


class SonicAnalysisRemoteOnlyProvider(AudioAnalysisProvider):
    """
    Sonic Analysis fallback for hosts that do not meet the local hardware requirements.

    Never imports torch/transformers; every session is forwarded to a remote worker. A
