...
from music_assistant.helpers.util import system_meets_requirements
...
class SonicAnalysisProvider(AudioAnalysisProvider):
    ...
    # Class-level default so instances built via __new__() (as several unit tests do, to
    # skip model loading) still see None rather than raising AttributeError.
    ...
    async def handle_async_init(self) -> None:
        ...
        if self._remote_client is not None:
            # Analysis runs on the configured worker; no local model to load.
            return
